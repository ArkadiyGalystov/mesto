...
const closePopupNowindows = (evt) => {
  if (evt.target.classList.contains('popup_opened')) {
    newCardOff(evt.target), profileOff(evt.target);
  };
  if (evt.target.classList.contains('slider_open')) {
    slideroff(evt.target);
  };
};
...
const closePopupEsc = (evt) => {
  if (evt.key === 'Escape') {
    newCardOff(evt.target), profileOff(evt.target), slideroff(evt.target);
  };
};
